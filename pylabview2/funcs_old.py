...
class Vectorized:
    ...
    def compute_conditions(df):
        ...
        import pandas as pd
        pd.read_pickle("/home/ubuntu/Dang/pickles/params_df.pkl")

        

        import pickle
        with open("/home/ubuntu/Dang/pickles/signal_dict.pkl", 'wb') as f:
    ...
    def compute_stats_v1():
        ...
        df_return = df_return.drop(['2024_08_01',])
    ...
    def compute_stats_test_old():
        # from tqdm import tqdm
        # sig_dic: dict = pd.read_pickle('/home/ubuntu/Dang/pickles/signal_dict.pkl')
    ...
    def compute_stats_v2():
        ...
        # Precompute return_num to avoid recomputation inside the loop
        # return_num_expanded = np.expand_dims(return_num, axis=0)
        return_num_expanded = return_num
        ...
        for i in tqdm(range(0, n-1)):
            ...
            for j in range(i+1, n):
                ...
                re = np.nan_to_num(return_num_expanded * cond, 0.0)
    ...
    @staticmethod
    def compute_stats_v3():
            from danglib.pylabview2.celery_worker import compute_multi_strategies, gen_plasma_functions, clean_redis
            from tqdm import tqdm
            clean_redis()
            _, _, psave, pload,  _ = gen_plasma_functions()
    ...
    def join_data():
        ...
        # wt_all = []
        for f in tqdm(fns):
            nt_ls, re_ls, wt_ls = pd.read_pickle(f)
            nt_all.append(nt_ls)
            # re_all.append(re_ls)
            # wt_all.append(wt_ls)

        to_pickle(nt_all, "/data/dang/nt_ls.pkl")

        nt_arr = np.vstack(nt_all)
        len(fns)

        for i in range(len(nt_all)):
            print(len(nt_all[i]))

        # ///////////
        df_nt = pd.read_pickle("/data/dang/nt_ls.pkl")
        df_nt = np.vstack(df_nt)
        pd.DataFrame(df_nt)
