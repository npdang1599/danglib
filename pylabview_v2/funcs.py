...
class Globs:
    ...
    def load_stocks_data():
        return Adapters.load_stocks_data_from_plasma()

    @staticmethod
    def get_one_stock_data(stock):
        """Get one stock data from df_stocks"""
        df_stocks = glob_obj.load_stocks_data()
        df: pd.DataFrame = df_stocks[df_stocks["stock"] == stock].copy()
...
def test():
    ...
    res, df = Scanner.scan_multiple_stocks(
            params=params,
            trade_direction='Long', 
            holding_periods=15,
        )    
